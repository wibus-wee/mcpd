...
// AutomaticMCPTool returns the MCP tool definition for mcpd.automatic_mcp.
func AutomaticMCPTool() mcp.Tool {
	return mcp.Tool{
		Name:        "mcpd.automatic_mcp",
...
// AutomaticEvalTool returns the MCP tool definition for mcpd.automatic_eval.
func AutomaticEvalTool() mcp.Tool {
	return mcp.Tool{
		Name:        "mcpd.automatic_eval",
		Description: "Invoke a concrete MCP tool that you previously discovered via mcpd.automatic_mcp. Pass the exact toolName, include arguments that match the advertised schema (JSON object), optionally add routingKey if you need sticky routing, and think of it as the execution phase.",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
