...
- **dev**: MCP Inspector with Go environment - Inspector launches and manages mcpd
...
- **Purpose**: Debug MCP protocol, launch and manage mcpd

### Prometheus
- **UI**: http://localhost:9500
- **Purpose**: Scrape and visualize mcpd metrics

### mcpd (launched by Inspector)
- **Metrics**: http://localhost:9090/metrics (when running)
...
3. Configure mcpd in Inspector:
   - **Command**: `go run /app/cmd/mcpd serve --config /app/dev/catalog.dev.yaml`
   - **Environment**: Add `MCPD_METRICS_ENABLED=true`
4. Click "Connect" to start mcpd
5. Inspector will show all MCP protocol messages

## Viewing Metrics

1. Ensure mcpd is running via Inspector
...
3. Restart mcpd in Inspector UI (disconnect/reconnect)
4. View protocol messages in Inspector
5. View metrics in Prometheus

## Configuration Files

- `dev/catalog.dev.yaml`: mcpd server configuration
...
- `9090`: mcpd Prometheus metrics
- `9500`: Prometheus UI

## Troubleshooting

**Inspector can't start mcpd:**
- Check that the command path is correct: `/app/cmd/mcpd`
- Verify environment variable: `MCPD_METRICS_ENABLED=true`
- Check logs: `docker compose logs dev`

**Prometheus shows no data:**
- Ensure mcpd is running (check Inspector)
