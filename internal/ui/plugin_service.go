...
func (s *PluginService) TogglePlugin(_ context.Context, req TogglePluginRequest) error {
	...
	if name == "" {
		return NewError(ErrCodeInvalidRequest, "Plugin name is required")
	}
	...
	if !found {
		return NewError(ErrCodeNotFound, fmt.Sprintf("Plugin %q not found", name))
	}
	...
	return NewError(ErrCodeNotImplemented, "Plugin enable/disable not yet implemented")
}
